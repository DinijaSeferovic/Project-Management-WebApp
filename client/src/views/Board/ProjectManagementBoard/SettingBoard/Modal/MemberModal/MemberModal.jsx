...
const MemberModal = ({
	...
	const addMember = (newMemberId, newMemberName) => {
		...
		setAlert(
			`${newMemberName} has been added to ${projectName}!`,
			"success"
		);
	};
		...
		<ModalContainer>
			<Container>
				<Content>
					...
					<Description>
						Add or remove project members. If you remove a member,
						the member will lose access to the project.
						<br />
						<span>&#42;</span> A project must have at least one
						member.
						<br />
					</Description>
					<InnerWrapper>
...
const memberOptions = (projectMembers, memberList, projectLead) =>
	memberList
		.filter(
			(member) =>
				!projectMembers.includes(member._id) &&
				member._id !== projectLead
		)
		.map((member) => ({
			key: member._id,
			value: member,
