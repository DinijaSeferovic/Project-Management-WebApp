...
const EpicList = ({
	...
	const handleDateChange = (difference) => {
		const newStartDate = moment(dateRange.startDate).add(
			difference,
			"days"
		);
	...
	const updateChildIssuesWithStatus = (columns, childIssues) => {
		return childIssues.map((issue) => {
			...
			const isFirstColumn =
				columnData && columnData.id === columnOrder[0];
		...
		>
			...
			>
				...
				{isChildIssuesVisible && (
					...
					>
						{updateChildIssuesWithStatus(columns, childIssues).map(
							(issue) => {
								...
								return (
									<ChildIssue
										key={issue._id}
										onClick={() =>
											openIssueDetailModal(issue.key)
										}
										style={{
											backgroundColor:
												epicColorProperty.bg,
										}}
									>
										<Icon
											type="user-icon"
